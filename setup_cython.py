...
密钥注入（避免把真实密钥提交进仓库）：
  ...
  开发默认值：seed="kk-mix-dev"，仅供本地调试，不要用于正式发布。
"""
...
def _load_secrets() -> str:
    ...
    if not seed:
        print("[setup_cython] 未提供 KK_LICENSE_SEED，使用开发默认值 kk-mix-dev。"
              "正式发布请务必设置！", file=sys.stderr)
    return seed or DEV_SEED
...
def _render_source() -> Path:
    seed_literal = _escape_bytes(_load_secrets().encode("utf-8"))

    text = SRC.read_text(encoding="utf-8")
    if "@@KK_LICENSE_SEED@@" not in text:
        raise SystemExit("_license_core.pyx 缺少占位符 @@KK_LICENSE_SEED@@")
