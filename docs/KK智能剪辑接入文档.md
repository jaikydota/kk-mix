...
巨量剪辑（本地 EXE 程序）需要接入视频混剪系统的用户认证体系，实现"登录后才能使用"的权限控制。
...
- 普通用户（`user`）：需要管理员在「用户管理」页面勾选「巨量剪辑 → 登录权限」后才能登录
...
| 400 | `"您没有巨量剪辑的使用权限，请联系管理员开通"` | 用户没有 KK 剪辑权限 |
