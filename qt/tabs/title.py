...
class TitleWorker(BatchWorker):
    ...
    def run_batch(self) -> tuple[bool, str]:
        ...
        bar = "=" * 60
        self.log(f"\n{bar}\n开始批量添加标题   视频数: {len(files)}")
        if self.title_lines:
            self.log(f"标题来源: TXT 共 {len(self.title_lines)} 行（不够循环）")
        else:
            self.log(f"标题: {self.fixed_title}")
        self.log(f"字体: {os.path.basename(self.font_path)}")
        self.log(bar)
        ...
        for idx, name in enumerate(files, 1):
            ...
            self.set_status(f"添加标题中 {idx}/{len(files)}")
...
class TitleTab(BaseTab):
    ...
    def build_form(self):
        ...
        self._add_folder_row(1, "输出文件夹", self.output_folder, is_output=True,
                             placeholder="留空则使用 源文件夹/title_output")
        ...
        if self._font_map:
            ...
            default = next((n for n in names if "微软雅黑" in n), None) or names[0]
            self.font_combo.setCurrentText(default)
        self._add_param_row(2, "字体", self.font_combo,
                            hint="含中文的字体可防止中文乱码")
        ...
        self.size_unit_hint = BodyLabel("% 视频高度", self)
        ...
        self._add_param_row(3, "字体大小", size_holder)
        ...
        self._add_param_row(4, "文字高度位置", self.y_percent,
                            hint="距顶部百分比，默认 8%")
        ...
        rb_fixed = RadioButton("固定文字", self); rb_fixed.setChecked(True)
        rb_txt = RadioButton("从 TXT 读取", self)
        ...
        self._add_param_row(6, "标题来源", mode_holder)

        # 固定文字
        self.fixed_text = LineEdit(self)
        self.fixed_text.setPlaceholderText("输入固定标题")
        self.form_layout.addWidget(BodyLabel("标题文字", self), 7, 0)
        self.form_layout.addWidget(self.fixed_text, 7, 1, 1, 2)

        # TXT 文件
        self.txt_file = LineEdit(self)
        self.txt_file.setPlaceholderText("每行一条标题，视频数超出时从第一行循环")
        self._add_file_row(8, "TXT 文件", self.txt_file,
    ...
    def _on_size_unit_change(self, btn_id: int, checked: bool):
        ...
        if unit == "percent":
            self.size_unit_hint.setText("% 视频高度")
            if self.fontsize.value() > 50:
                self.fontsize.setValue(5.0)
        else:
            self.size_unit_hint.setText("px 像素")
    ...
    def build_worker(self):
        ...
        if not font_path or not os.path.exists(font_path):
            InfoBar.error("字体不存在", f"找不到字体文件：{font_label}",
        ...
        if text_mode == "fixed":
            ...
            if not txt_file or not os.path.exists(txt_file):
                InfoBar.error("TXT 文件不存在", txt_file or "(空)",
                              parent=self.main, position=InfoBarPosition.TOP)
                return None
            try:
                ...
                InfoBar.error("读取 TXT 失败", str(e),
                              parent=self.main, position=InfoBarPosition.TOP)
                return None
            if not title_lines:
                InfoBar.error("TXT 内容为空", "文件中没有有效行",
                              parent=self.main, position=InfoBarPosition.TOP)
                return None
            fixed_title = None
