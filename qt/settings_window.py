...
class SettingsDialog(MessageBoxBase):
    ...
    def __init__(self, main_window):
        ...
        # 标题
        self.title_label = SubtitleLabel(tr("全局设置"), self)
    ...
    def _build_tts_card(self, layout: QVBoxLayout):
        card, g = _section(tr("TTS 语音服务"), self)
        g.addWidget(BodyLabel(tr("服务地址:"), card), 0, 0)
        ...
        hint = BodyLabel(tr("用于「字幕转场拼接」的配音合成"), card)
    ...
    def _build_debug_card(self, layout: QVBoxLayout):
        card, g = _section(tr("调试选项"), self)
        self.verbose_log = CheckBox(tr("打印详细日志"), card)
    ...
    def _build_perf_card(self, layout: QVBoxLayout):
        card, g = _section(tr("性能与稳定性"), self)

        g.addWidget(BodyLabel(tr("并行线程数:"), card), 0, 0)
        ...
        hint = BodyLabel(tr("同时处理文件数（建议 1-4）"), card)
        hint.setStyleSheet("color: #8a8a8a;")
        g.addWidget(hint, 0, 2)

        self.speed_priority = CheckBox(tr("极速模式（ultrafast，编码更快，清晰度略降）"), card)
        self.speed_priority.setChecked(self.settings.speed_priority)
        g.addWidget(self.speed_priority, 1, 0, 1, 3)

        self.compress_video = CheckBox(tr("启用压缩（指定码率，否则使用 CRF 23）"), card)
        ...
        br_layout.addWidget(BodyLabel(tr("码率:"), card))
        ...
        br_hint = BodyLabel(tr("最大 50M，越大画质越好"), card)
    ...
    def _build_license_card(self, layout: QVBoxLayout):
        card, g = _section(tr("授权信息"), self)
        if _license_core is None:
            g.addWidget(BodyLabel(tr("当前为开发模式（未编译 _license_core.pyd）"), card), 0, 0)
            layout.addWidget(card)
            return
        ...
        mid_h.setSpacing(8)
        mid_label = BodyLabel(tr("本机机器码：{0}").format(mid), card)
        mid_h.addWidget(mid_label)
        copy_btn = PushButton(tr("复制"), card)
        ...
        try:
            ...
            if saved:
                ...
                if ok:
                    ...
                    expire_str = info.get("expire_date", tr("未知"))
                    expire_dt = datetime.strptime(expire_str, "%Y-%m-%d %H:%M:%S")
                    days_left = (expire_dt - datetime.now()).days
                    g.addWidget(BodyLabel(tr("到期时间：{0}").format(expire_str), card), 1, 0)
                    color = "#107c10" if days_left > 7 else ("#e67e00" if days_left > 0 else "#c42b1c")
                    remain_label = BodyLabel(tr("剩余天数：{0} 天").format(days_left), card)
                    remain_label.setStyleSheet(f"color: {color}; font-weight: 600;")
                    g.addWidget(remain_label, 2, 0)
                else:
                    err_label = BodyLabel(tr("授权状态：{0}").format(msg), card)
                    err_label.setStyleSheet("color: #c42b1c;")
                    g.addWidget(err_label, 1, 0)
            else:
                g.addWidget(BodyLabel(tr("授权状态：未激活"), card), 1, 0)
        except Exception:
            g.addWidget(BodyLabel(tr("无法读取授权信息"), card), 1, 0)
    ...
    def _on_save(self):
        ...
        try:
            ...
            self.main.log(tr("[设置] 已保存到 settings.json"))
